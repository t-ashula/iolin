use strict;
use warnings;
use Test::More tests => 6;

use Net::OperaLink::Data::SpeedDial;

{
  my $h = Net::OperaLink::Data::SpeedDial->new;
  is $h->to_opera_link_xml(), "";
}

my $sd2_add = q{<speeddial status="added" position="1"><icon></icon><reload_only_if_expired>0</reload_only_if_expired><reload_enabled>0</reload_enabled><reload_interval>0</reload_interval><uri>http://redir.opera.com/speeddials/portal/</uri><title>Opera Portal beta</title></speeddial>};

{
  my $h = Net::OperaLink::Data::SpeedDial->new;
  $h->add( { position => 1, uri => 'http://redir.opera.com/speeddials/portal/', title => 'Opera Portal beta' } );
  is $h->to_opera_link_xml(), $sd2_add;
}

my $sd2_del = q{<speeddial status="deleted" position="1" />};
{
  my $h = Net::OperaLink::Data::SpeedDial->new;
  $h->add( { position => 1, uri => 'http://redir.opera.com/speeddials/portal/', title => 'Opera Portal beta' } );
  $h->del( { position => 1, uri => 'http://redir.opera.com/speeddials/portal/' } );
  is $h->to_opera_link_xml(), $sd2_del;
}

my $sd3_add = q{<speeddial status="added" position="1"><icon></icon><reload_only_if_expired>0</reload_only_if_expired><reload_enabled>1</reload_enabled><reload_interval>3600</reload_interval><uri>http://redir.opera.com/speeddials/portal/</uri><title>Opera Portal beta</title></speeddial>};
{
  my $h = Net::OperaLink::Data::SpeedDial->new;
  $h->add( { 
	    position => 1, 
	    uri => 'http://redir.opera.com/speeddials/portal/', 
	    title => 'Opera Portal beta', 
	    reload_enabled => 1,
	    reload_only_if_expired => 0,
	    reload_interval => 3600,
	   } );
  is $h->to_opera_link_xml(), $sd3_add;
}

my $sd4_add = q{<speeddial status="added" position="1"><icon></icon><reload_only_if_expired>1</reload_only_if_expired><reload_enabled>1</reload_enabled><reload_interval>0</reload_interval><uri>http://redir.opera.com/speeddials/portal/</uri><title>Opera Portal beta</title></speeddial>};
{
  my $h = Net::OperaLink::Data::SpeedDial->new;
  $h->add( { 
	    position => 1, 
	    uri => 'http://redir.opera.com/speeddials/portal/', 
	    title => 'Opera Portal beta', 
	    reload_enabled => 1,
	    reload_only_if_expired => 1,
	    reload_interval => 0,
	   } );
  is $h->to_opera_link_xml(), $sd4_add;
}

my $sd1_add = q{<speeddial status="added" position="7"><icon></icon><reload_only_if_expired>0</reload_only_if_expired><reload_enabled>0</reload_enabled><reload_interval>0</reload_interval><uri>http://d.hatena.ne.jp/t_ashula/</uri><title>t. @ hatena</title></speeddial>};
{
  my $h = Net::OperaLink::Data::SpeedDial->new;
  $h->add( { 
	    position => 7, 
	    uri => 'http://d.hatena.ne.jp/t_ashula/', 
	    title => 't. @ hatena', 
	    reload_enabled => 0,
	    reload_only_if_expired => 0,
	    reload_interval => 0,
	    icon => ''
	   } );
  is $h->to_opera_link_xml(), $sd1_add;
}

__END__




